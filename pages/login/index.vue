		...
		<view class="wrapper">
			<view class="input-content">
				<view class="cu-form-group" style="border: 2upx solid whitesmoke;margin-bottom: 20px;border-radius: 30px">
					...
					<input v-model="username" type="text" placeholder="请输入手机号" maxlength="11" />
				</view>
	...
	export default {
		...
		methods: {
			...
			toLogin() {
				...
				else if (!this.cs.test(this.username)){
					this.$Raichu.showToast("手机号输入格式不正确")
				}
				else if (!this.password) {
					this.$Raichu.showToast("请输入密码");
				} else {
					this.$Raichu.showLoading("正在登录中...");
					this.$Request.postT("/externalApi/login", {
					username:this.username,
			 		password: this.password,
						...
						if (res.id) {
							...
							this.$Raichu.showToast(res.message)
						}
					});
				}
			},
